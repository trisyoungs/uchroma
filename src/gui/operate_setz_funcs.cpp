...
bool OperateSetZDialog::setZFromSourceFiles()
{
	...
	for (DataSet* dataSet = targetCollection_->dataSets(); dataSet != NULL; dataSet = dataSet->next)
	{
		...
		LineParser parser(dataSet->sourceFileName());
		if (!parser.ready())
		{
			msg.print("No filename is set for dataset '%s' (and source = FileSource).\n", qPrintable(dataSet->name()));
			parser.closeFiles();
			++nFailed;
			continue;
		}
		parser.skipLines(ui.FromSourceFilesLineSpin->value()-1);
		if (!parser.getLine(text))
		{
			msg.print("Ran out of lines when getting data from file '%s'.\n", qPrintable(dataSet->sourceFileName()));
			++nFailed;
			continue;
		}
	...
	if (ui.FromSourceFilesDateTimeCheck->isChecked())
	{
		for (DataSet* dataSet = targetCollection_->dataSets(); dataSet != NULL; dataSet = dataSet->next) targetCollection_->setDataSetZ(dataSet, dataSet->data().z() - earliest);
	}
...
void OperateSetZDialog::updateFromSourceFilesData(bool reReadLine)
{
	...
	for (DataSet* dataSet = targetCollection_->dataSets(); dataSet != NULL; dataSet = dataSet->next)
	{
		...
		parser.skipLines(ui.FromSourceFilesLineSpin->value()-1);
		if (!parser.getLine(sourceFilesTestText_)) continue;
