/*
	*** EditState - AXes
	*** src/session/editstate_axes.cpp
...
bool EditState::changeAxes(const List<EditStateData>& data, bool revert)
{
	...
	if (!axesTarget)
	{
		msg.print("Internal Error - Axes target with id %i no longer exists.\n", targetObject_.id());
		return false;
	}
	...
	switch (targetQuantity_)
	{
		...
		case (EditState::AxesVisibleQuantity):
			...
			break;
		default:
			msg.print("Internal Error: Target quantity %i for Axes object not handled in EditState::changeAxes().\n", targetQuantity_);
			return false;
	}
	return result;
}
